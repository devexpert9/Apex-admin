...
export class ForgotPasswordComponent implements OnInit, OnDestroy {
	...
	submit()
	{
		...
		this.adminService.postData('forgotPasswordAdmin',{'email':email}).subscribe((response:any) => {
			if(response.status == 1){
				this.toastr.success("We have sent you an email. Please check.", 'Success');
				this.router.navigateByUrl('/auth/login');
			}else{
				this.toastr.error("Entered email is not exist in our system.", 'Error');
			}
      	})
